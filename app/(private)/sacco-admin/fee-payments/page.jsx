            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex gap-3">
                    <Button 
                        ...
                        className="bg-amber-600 hover:bg-amber-700 text-white font-black px-6 h-12 rounded-xl shadow-xl shadow-amber-100 flex items-center gap-2 transition-all active:scale-95"
                    >
            ...
            <div className="bg-white rounded-[2rem] p-4 shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4">
                <div className="flex-1 relative group">
                    ...
                    <Input 
                        ...
                        className="pl-14 h-14 rounded-2xl border-slate-100 focus:border-amber-600 bg-slate-50/50 shadow-none border-0 text-base font-medium"
                        value={searchTerm}
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-slate-50/50 border-b border-slate-100">
                                <TableHead className="pl-12 py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Payer Details</TableHead>
                                <TableHead className="py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Requirement</TableHead>
                                <TableHead className="py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Outstanding Balance</TableHead>
                                <TableHead className="py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Status</TableHead>
                                <TableHead className="pr-12 py-7 text-right text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Profile</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                                    ...
                                    <TableRow key={acc.reference} className="hover:bg-amber-50/30 transition-all border-b border-slate-50 last:border-0 group h-24">
                                        <TableCell className="pl-12">
                                            <div className="flex flex-col">
                                                <span className="font-black text-slate-800 text-base tracking-tight group-hover:text-amber-700 transition-colors">{acc.member?.first_name} {acc.member?.last_name}</span>
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[11px] font-black text-slate-600 uppercase tracking-tight">{acc.fee_type?.name}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-center font-black text-slate-800 font-mono text-lg tracking-tighter">
                                            {Number(acc.outstanding_balance).toLocaleString()}
                                        </TableCell>
                                        <TableCell className="text-center">
                                            <span className={`px-3 py-1.5 rounded-full text-[9px] font-black tracking-widest border-2 shadow-sm ${
                                        ...
                                        <TableCell className="pr-12 text-right">
                                            <Link href={`/sacco-admin/members/${acc.member?.id}`}>
                                                <Button size="icon" variant="ghost" className="rounded-2xl hover:bg-white border text-slate-300 hover:text-amber-700 hover:border-amber-100 h-10 w-10 transition-all">
            ...
            <Dialog open={isBulkPaymentOpen} onOpenChange={setIsBulkPaymentOpen}>
                <DialogContent className="max-w-[1200px] bg-slate-50 border-none rounded-[4rem] p-0 overflow-hidden shadow-2xl">
                    <DialogHeader className="bg-white px-12 py-10 border-b border-slate-100">
                        <DialogTitle className="text-3xl font-black text-amber-800 tracking-tighter flex items-center gap-3">
                           <CreditCard className="w-8 h-8 p-1.5 bg-amber-100 rounded-lg" /> Multi-Fee Collection Engine
                        </DialogTitle>
                    ...
                    <div className="p-12">
                        <Tabs defaultValue="form" className="w-full">
                            <TabsList className="bg-white border p-2 rounded-2xl h-16 mb-10 max-w-sm shadow-inner flex items-stretch">
                                <TabsTrigger value="form" className="flex-1 rounded-xl data-[state=active]:bg-amber-600 data-[state=active]:text-white font-black text-xs uppercase tracking-widest transition-all">
                                    <Plus className="w-4 h-4 mr-2" /> Manual Batch
                                </TabsTrigger>
                                <TabsTrigger value="upload" className="flex-1 rounded-xl data-[state=active]:bg-amber-600 data-[state=active]:text-white font-black text-xs uppercase tracking-widest transition-all">
