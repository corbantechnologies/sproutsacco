            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex gap-3">
                    <Button 
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-6 h-12 rounded-xl shadow-xl shadow-emerald-100 flex items-center gap-2 transition-all active:scale-95"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <Card className="border-none shadow-sm bg-emerald-600 text-white rounded-[2rem] p-6">
                    <CardHeader className="p-0 pb-2">
                        ...
                        <CardTitle className="text-3xl font-black tracking-tighter">{accounts?.length || 0}</CardTitle>
                    </CardHeader>
                </Card>
                <Card className="border-none shadow-sm bg-white rounded-[2rem] p-6 border-b-4 border-b-blue-500">
                    <CardHeader className="p-0 pb-2">
                        ...
                        <CardTitle className="text-2xl font-black text-slate-800 font-mono">
            ...
            <div className="bg-white rounded-[2rem] p-4 shadow-sm border border-slate-100">
                <div className="relative group">
                    ...
                    <Input 
                        ...
                        className="pl-14 h-14 rounded-2xl border-slate-100 focus:border-emerald-600 bg-slate-50/50 shadow-none border-0 text-base font-medium"
                        value={searchTerm}
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-slate-50/50 border-b border-slate-100">
                                <TableHead className="pl-12 py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Contributor</TableHead>
                                <TableHead className="py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Product Line</TableHead>
                                <TableHead className="py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Net Balance</TableHead>
                                <TableHead className="py-7 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Status</TableHead>
                                <TableHead className="pr-12 py-7 text-right text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Actions</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {filteredAccounts.length > 0 ? (
                                    ...
                                    <TableRow key={acc.reference} className="hover:bg-emerald-50/30 transition-all border-b border-slate-50 last:border-0 group h-24">
                                        <TableCell className="pl-12">
                                            <div className="flex flex-col">
                                                <span className="font-black text-slate-800 text-base tracking-tight group-hover:text-emerald-700 transition-colors">{acc.member?.first_name} {acc.member?.last_name}</span>
                                                <span className="text-[11px] font-bold text-slate-400 font-mono italic tracking-tighter uppercase">{acc.account_number}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="flex flex-col">
                                                <span className="text-[11px] font-black text-slate-600 uppercase tracking-tight">{acc.product?.name}</span>
                                        ...
                                        <TableCell className="text-center">
                                            <span className="px-3 py-1.5 rounded-full text-[9px] font-black tracking-widest border-2 bg-emerald-50 text-emerald-700 border-emerald-100 shadow-sm">
                                                ACTIVE
                                            </span>
                                        </TableCell>
                                        <TableCell className="pr-12 text-right">
                                            <div className="flex justify-end gap-2">
                                                <Link href={`/sacco-admin/members/${acc.member?.id}`}>
                                                    <Button size="icon" variant="ghost" className="rounded-2xl hover:bg-white border text-slate-300 hover:text-emerald-700 hover:border-emerald-100 h-10 w-10">
                                ...
                                <TableRow>
                                    <TableCell colSpan={5} className="text-center py-24 text-slate-300 font-black uppercase tracking-[0.6em] italic opacity-50">
            ...
            <Dialog open={isBulkDepositOpen} onOpenChange={setIsBulkDepositOpen}>
                <DialogContent className="max-w-[1200px] bg-slate-50 border-none rounded-[4rem] p-0 overflow-hidden shadow-2xl">
                    <DialogHeader className="bg-white px-12 py-10 border-b border-slate-100 flex flex-row items-center justify-between">
                        <div>
                            <DialogTitle className="text-3xl font-black text-emerald-800 tracking-tighter flex items-center gap-3">
                               <ArrowDownLeft className="w-8 h-8 p-1.5 bg-emerald-100 rounded-lg" /> Bulk Funds Collection
                            </DialogTitle>
                    ...
                    <div className="p-12">
                        <Tabs defaultValue="form" className="w-full">
                            <TabsList className="bg-white border p-2 rounded-2xl h-16 mb-10 max-w-sm shadow-inner flex items-stretch">
                                <TabsTrigger value="form" className="flex-1 rounded-xl data-[state=active]:bg-emerald-600 data-[state=active]:text-white font-black text-xs uppercase tracking-widest transition-all">
                                    <Plus className="w-4 h-4 mr-2" /> Manual Data
                                </TabsTrigger>
                                <TabsTrigger value="upload" className="flex-1 rounded-xl data-[state=active]:bg-emerald-600 data-[state=active]:text-white font-black text-xs uppercase tracking-widest transition-all">
