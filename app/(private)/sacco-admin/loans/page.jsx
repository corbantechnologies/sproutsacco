            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tighter flex items-center gap-3">
                        <div className="bg-[#174271] p-2 rounded-xl text-white shadow-lg">
                ...
                <div className="flex gap-3">
                    <Button 
                        ...
                        className="bg-[#ea1315] hover:bg-[#c71012] text-white font-black px-6 h-12 rounded-xl shadow-xl shadow-rose-100 flex items-center gap-2 transition-all active:scale-95"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="border-none shadow-sm bg-[#174271] text-white rounded-[2rem] p-6">
                    <CardHeader className="p-0 pb-2">
                        ...
                        <CardTitle className="text-3xl font-black">{loans?.length || 0}</CardTitle>
                    </CardHeader>
                </Card>
            ...
            <div className="bg-white rounded-[2rem] p-4 shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4">
                <div className="flex-1 relative group">
                    ...
                    <Input 
                        ...
                        className="pl-12 h-12 rounded-2xl border-slate-100 focus:border-[#174271] bg-slate-50 shadow-none border-0"
                        value={searchTerm}
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-slate-50/50 border-b border-slate-100">
                                <TableHead className="pl-10 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Account Owner</TableHead>
                                <TableHead className="py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Loan Product</TableHead>
                                <TableHead className="py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Principal</TableHead>
                                <TableHead className="py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Balance</TableHead>
                                <TableHead className="py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">Status</TableHead>
                                <TableHead className="pr-10 py-6 text-right text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Quick View</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {filteredLoans.length > 0 ? (
                                    ...
                                    <TableRow key={loan.reference} className="hover:bg-slate-50/50 transition-all border-b border-slate-50 last:border-0 group h-24">
                                        <TableCell className="pl-10">
                                            <div className="flex flex-col">
                                                <span className="font-black text-slate-800 text-base tracking-tight">{loan.member?.first_name} {loan.member?.last_name}</span>
                                                <span className="text-[11px] font-bold text-slate-400 font-mono italic">{loan.account_number}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="inline-flex items-center gap-2 bg-[#174271]/5 px-3 py-1 rounded-lg border border-[#174271]/10">
                                                <span className="text-[11px] font-black text-[#174271] uppercase">{loan.product?.name || "Product"}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-center font-black text-slate-800 font-mono">
                                            {Number(loan.principal).toLocaleString()}
                                        </TableCell>
                                        <TableCell className="text-center font-black text-[#ea1315] font-mono">
                                            {Number(loan.balance).toLocaleString()}
                                        </TableCell>
                                        <TableCell className="text-center">
                                            <span className={`px-3 py-1.5 rounded-full text-[10px] font-black tracking-widest border-2 flex items-center justify-center gap-1.5 w-fit mx-auto ${
                                        ...
                                        <TableCell className="pr-10 text-right">
                                            <Link href={`/sacco-admin/members/${loan.member?.id}/${loan.reference}`}>
                                                <Button size="icon" variant="ghost" className="rounded-2xl hover:bg-white border-2 border-transparent hover:border-slate-100 shadow-none h-11 w-11 transition-all group-hover:text-[#174271]">
                                ...
                                <TableRow>
                                    <TableCell colSpan={6} className="text-center py-20 text-slate-300 font-black uppercase tracking-[0.5em] italic">
            ...
            <Dialog open={isBulkDisburseOpen} onOpenChange={setIsBulkDisburseOpen}>
                <DialogContent className="max-w-[1200px] bg-slate-50 border-none rounded-[3rem] p-0 overflow-hidden shadow-2xl">
                    <DialogHeader className="bg-white px-10 py-8 border-b border-slate-100">
                        <DialogTitle className="text-2xl font-black text-[#174271] tracking-tighter flex items-center gap-2">
                    ...
                    <div className="p-8">
                        <Tabs defaultValue="form" className="w-full">
                            <TabsList className="bg-white border p-1 rounded-2xl h-14 mb-8 max-w-md shadow-sm">
                                <TabsTrigger value="form" className="flex-1 rounded-xl data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 font-black text-[10px] uppercase tracking-widest">
                                    <Plus className="w-4 h-4 mr-2" /> Manual Batch
                                </TabsTrigger>
                                <TabsTrigger value="upload" className="flex-1 rounded-xl data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 font-black text-[10px] uppercase tracking-widest">
